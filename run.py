...
def main(cfg, args):
    ...
    # get datamodule
    dm = mica.builder.build_data_module(cfg)

    # define lightning module
    model = mica.builder.build_lightning_model(cfg, dm)
    ...
    if args.load_path != "":
        if os.path.exists(args.load_path):
            model = model.load_from_checkpoint(args.load_path)
            print(f"load checkpoint: {args.load_path} successfully!")
        else:
            raise RuntimeError("Please input a correct checkpoint path.")
    ...
    if args.test:
        ...
        print("read ckpt: ", cfg.model.checkpoint)
        trainer.test(model=model, datamodule=dm, ckpt_path=ckpt_path)  # NOTE: add ckpt_path
...
if __name__ == "__main__":
    ...
    for split in np.arange(args.splits):
        ...
        cfg.output_dir = f"./out/{cfg.experiment_name}/{cfg.extension}"
